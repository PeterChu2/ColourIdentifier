...
public class QueryColour implements Runnable {
	...
	public QueryColour(Context c, ColourBoxView colourBox) {
	...
	public void setRawSQL(int r, int g, int b) {
		// round values to the nearest 10, as the database only supports these
		// values
		r = round10(r);
		g = round10(g);
		b = round10(b);
		this.rawsql = String.format("SELECT text FROM RGBValues WHERE red=%d and green=%d AND blue=%d",
						r, g, b);
	}

	// rounding function to the nearest 10
	private static int round10(int num) {
		// ensure that value does not round to 260, as the max value is 256 for
		// RGB
		if (num >= 255) {
			return 250;
		}
	...
	public void createRawSQL(byte[] data, int width, int height, int index) {
		...
		this.setRawSQL((int) (Color.red(rgb[index])),
				(int) (Color.green(rgb[index])), (int) (Color.blue(rgb[index])));
	...
	public synchronized void refreshStatus() {
		if (rawsql != null) {
			...
			if (cursor.moveToFirst()) {
				try {
					ColourBoxView.setColour(colour);
